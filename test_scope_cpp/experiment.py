...
def task(model, n, p, k, seed, relax_ratio):
    ...
    grad_jax_data = lambda x: jax.grad(model_dict[model].loss_jax)(x, data)
    ...
    for method, solver in {
        ...
        if method == "SCOPE":
            ...
            solver.solve(loss_jax_data, gradient=grad_jax_data)
...
if __name__ == "__main__":
    ...
    print(task(model="linear", n=200, p=50, k=5, seed=11, relax_ratio=1.0))
    print(task(model="logistic", n=100, p=50, k=5, seed=1099, relax_ratio=1.0))
    print(task(model="ising", n=200, p=190, k=40, seed=708, relax_ratio=1.0))
