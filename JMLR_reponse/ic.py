...
def task(model: str, sample_size, dim, sparsity_level, seed):
    ...
    for method, solver in {
        "SCOPE": ScopeSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
        "GraSP": GraspSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
        "FoBa": FobaSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
        "OMP": OMPSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
    }.items():
        for is_jit in [True]:  # , False]:
            t1 = time.time()
            solver.solve(loss_jax, jit=is_jit)
            t2 = time.time()
            results.append(
                {
                    "method": method,
    ...
    # IHT, HTP
    for method, solver in {
        "IHT": IHTSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
        "HTP": HTPSolver(dim, sparsity=range(dim // 5), sample_size=sample_size, ic_method=ic_method),
    }.items():
        for is_jit in [True]:  # , False]:
            t1 = time.time()
            best_loss = np.inf
    ...
    m = linear_model.LassoLarsIC(fit_intercept=False, criterion="aic")
    m.fit(data[0], data[1])
    params = m.coef_
    ...
    ic_value = ic_method(
        objective_value=loss_jax(ht_params),
        dimensionality=dim,
        effective_params_num=len(support_set),
        train_size=sample_size,
    )
    print(m.alpha_, loss_jax(ht_params), ic_value, len(support_set))
    """
    x = cp.Variable(dim)
    ...
    best_ic = np.inf
    start = time.time()
    for target_sparsity in range(dim // 5 - 1, -1, -1):
        try:
            problem.solve()
        except:
            return results
        ...
        ht_params[list(support_set)] = params[list(support_set)]
        if model == "linear":
            ic_value = sample_size * np.log(loss_jax(ht_params)) + len(support_set) * 2  # np.log(sample_size)
        else:
