...
logistic = r %>% 
  ...
  t() %>% 
  as_tibble()
colnames(logistic) = c('n', 'seed', 'Lasso_accuracy', 'Lasso_best_lambda', 'Lasso_time', 'GraHTP_accuracy', 'GraHTP_time', 'GraHTP_cv_accuracy', 'GraHTP_best_step_size', 'GraHTP_cv_time', 'GraSP_accuracy', 'GraSP_time', 'SCOPE_accuracy', 'SCOPE_time')
logistic = logistic %>% 
  mutate(across(everything(), as.numeric)) %>%
  filter(!is.na(Lasso_accuracy)) %>% 
  filter(n%%100==0)

r = readLines("/data/home/wangzz/github/splicing-universal-experiment/total_experiment/experiment/ising.out")
r = r[str_starts(r,'\\{')]
ising = r %>% 
  str_extract_all("(-?[0-9]\\d*\\.?\\d*e?-?\\d*)|nan") %>% 
  sapply(as.matrix) %>% 
  t() %>% 
  as_tibble()
colnames(ising) = c('n', 'k', 'seed', 'Lasso_accuracy', 'Lasso_best_lambda', 'Lasso_time', 'GraHTP_accuracy', 'GraHTP_time', 'GraHTP_cv_accuracy', 'GraHTP_best_step_size', 'GraHTP_cv_time', 'GraSP_accuracy', 'GraSP_time', 'SCOPE_accuracy', 'SCOPE_time')
ising = ising %>% 
  mutate(across(everything(), as.numeric)) %>%
  filter(!is.na(Lasso_accuracy), k==40)
```
```{r}
ising %>% group_by(n) %>% count()
```
```{r}
write_csv(ising, "ising_tidy.csv")
```


```{r}
ising_tidy = read_csv("ising_tidy.csv")
```
